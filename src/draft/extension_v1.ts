...
export function activate(context: vscode.ExtensionContext) {
    ...
    const generateMessageCommand = vscode.commands.registerCommand(COMMANDS.GENERATE_MESSAGE, async (webviewView: vscode.WebviewView) => {
        ...
        try {
            const message = await fetchMessage({ backendUrl, authToken, format, commitType, length: maxLenInLine, apiKey, prompt, model, diff });
...
async function fetchMessage(config: {
    ...
    const { backendUrl, authToken, format, commitType, length, apiKey, prompt, model, diff } = config;
    try {
